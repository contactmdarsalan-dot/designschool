...
def send_whatsapp_otp(phone_number, otp_code):
    ...
    if content_sid:
        ...
        payload["Body"] = (
            f"Your Design School verification code is {otp_code}. It expires in 5 minutes."
        )
...
class GoogleLoginView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if not email:
            return Response(
                {"error": "Email not found in Google token"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AdminDashboardView(APIView):
    ...
    def get(self, request):
        ...
        recent_enrollments = Enrollment.objects.select_related("course").order_by("-created_at")[:5]
        recent_callbacks = CallbackRequest.objects.order_by("-created_at")[:5]
        upcoming_assignments = Assignment.objects.select_related("course").order_by("due_date")[:5]
        recent_posts = BlogPost.objects.select_related("category", "author").order_by(
            "-updated_at"
        )[:5]
        ...
        total_revenue = sum(
            (get_course_price(item.course) for item in verified_enrollments), Decimal("0")
        )
        month_revenue = sum(
            (
                get_course_price(item.course)
                for item in verified_enrollments
                if item.created_at >= current_month_start
            ),
            Decimal("0"),
        )
        week_revenue = sum(
            (
                get_course_price(item.course)
                for item in verified_enrollments
                if item.created_at >= current_week_start
            ),
            Decimal("0"),
        )
        ...
        category_breakdown = [
            {
                ...
                "share": round(((item.enrollment_total or 0) / category_total) * 100)
                if category_total
                else 0,
        ...
        top_instructors = [
            {
                ...
                "email": item.email,
                "expertise": getattr(getattr(item, "mentor_profile", None), "expertise", "")
                or "Mentor",
                "company": getattr(getattr(item, "mentor_profile", None), "current_company", "")
                or "Design School",
                "course_total": item.course_total or 0,
        ...
        lead_sources = [
            {
                "label": enquiry_labels.get(
                    item["enquiry_for"], item["enquiry_for"].replace("_", " ").title()
                ),
                "value": item["total"],
        ...
        return Response(
            {
                "data": {
                    "stats": {
                        ...
                        "callback_requests": CallbackRequest.objects.exclude(
                            status="resolved"
                        ).count(),
