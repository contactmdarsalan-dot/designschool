...
def build_course_pricing(course):
    sale_price = course.discounted_price if course.is_discount_active and course.discounted_price else course.actual_price
    return {
...
def build_join_course_payload(request):
    ...
    payment_methods = list(PaymentMethod.objects.filter(is_active=True).order_by('sort_order', 'name'))
...
def build_student_workspace_payload(request):
    ...
    for enrollment in active_enrollments:
        ...
        next_session = None
        for session in sorted(course_sessions, key=lambda item: (item.date, item.start_time or time.min)):
            if session.date >= today:
                next_session = {
                    'id': session.id,
    ...
    return {
        ...
        'overview': {
            ...
            'certificates_earned': sum(1 for item in certificate_rows if item['status'] == 'issued'),
...
class StudentJoinCourseView(APIView):
    ...
    def post(self, request):
        ...
        if not first_name or not last_name:
            raise ValidationError({'profile': 'Complete your first and last name before requesting enrollment.'})
        if not whatsapp_number:
            raise ValidationError({'whatsapp_number': 'WhatsApp number is required to request enrollment.'})
