    ...
    if request.method == "POST":
        ...
        try:
            ...
            html_content = render_to_string(
                "students/emails/forgot_password_email.html",
                {
                    "user": user,
                    "reset_url": reset_url,
                },
...
def student_dashboard(request):
    ...
    enrollments = Enrollment.objects.filter(email=user.email, status="verified").select_related(
        "course"
    )
    enrolled_courses = [en.course for en in enrollments]  # list of actual Course objects

    # Filter related data safely
    recent_assignments = StudentAssignment.objects.filter(student=user).order_by("-submitted_at")[
        :5
    ]
    ...
    attendance_percentage = (
        (
            ...
            / StudentAttendance.objects.filter(student=user).count()
        )
        * 100
        if StudentAttendance.objects.filter(student=user).exists()
        else 0
    )
...
def my_learning(request):
    ...
    return render(request, "students/my_learning.html", {"enrolled_courses": enrolled_courses})
...
def student_certificates(request):
    ...
    return render(
        request,
        "students/student_certificates.html",
...
def send_certificate_email(student_certificate_id):
    ...
    cert_url = (
        f"{settings.MEDIA_URL}{certificate.download_link}" if certificate.download_link else None
    )
