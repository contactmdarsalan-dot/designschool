...
class FreeResource(models.Model):
    ...
    resource_type = models.CharField(
        max_length=60, choices=RESOURCE_TYPE_CHOICES, default="resource_pack"
    )
...
class FreeResourceWorkflowStep(models.Model):
    resource = models.ForeignKey(
        FreeResource, on_delete=models.CASCADE, related_name="workflow_steps"
    )
    title = models.CharField(max_length=120)
    detail = models.TextField(max_length=280)
    sort_order = models.PositiveIntegerField(default=0)
...
class CallbackRequest(models.Model):
    ...
    source_page = models.CharField(
        max_length=140, blank=True, help_text="Example: /request-callback"
    )
