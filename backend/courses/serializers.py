...
class CourseSerializer(serializers.ModelSerializer):
    ...
    def _replace_nested_content(self, course, nested_data):
        if nested_data.get("learning_points") is not None:
            self._replace_text_items(
                course, "learning_points", WhatYouWillLearn, nested_data["learning_points"]
            )
        if nested_data.get("requirements") is not None:
            self._replace_text_items(
                course, "requirements", Requirement, nested_data["requirements"]
            )
        if nested_data.get("target_audience") is not None:
            self._replace_text_items(
                course, "target_audience", WhoIsFor, nested_data["target_audience"]
            )
        if nested_data.get("detail_facts") is not None:
            self._replace_sorted_items(
                course, "detail_facts", CourseDetailFact, nested_data["detail_facts"]
            )
        if nested_data.get("skill_outcomes") is not None:
            self._replace_sorted_items(
                course, "skill_outcomes", CourseSkillOutcome, nested_data["skill_outcomes"]
            )
        ...
        if nested_data.get("audience_items") is not None:
            self._replace_sorted_items(
                course, "audience_items", CourseAudienceItem, nested_data["audience_items"]
            )
        ...
        if nested_data.get("comparison_points") is not None:
            self._replace_sorted_items(
                course, "comparison_points", CourseComparisonPoint, nested_data["comparison_points"]
            )
        ...
        if nested_data.get("builder_items") is not None:
            self._replace_sorted_items(
                course, "builder_items", CourseBuilderItem, nested_data["builder_items"]
            )
        if nested_data.get("certificate_points") is not None:
            self._replace_sorted_items(
                ...
                "certificate_points",
                CourseCertificatePoint,
                nested_data["certificate_points"],
            )
        if nested_data.get("mentor_spotlights") is not None:
            self._replace_sorted_items(
                course, "mentor_spotlights", CourseMentorSpotlight, nested_data["mentor_spotlights"]
            )

    @transaction.atomic
    def create(self, validated_data):
        nested_data = {field: validated_data.pop(field, []) for field in self.nested_json_fields}
    ...
    def update(self, instance, validated_data):
        nested_data = {field: validated_data.pop(field, None) for field in self.nested_json_fields}
