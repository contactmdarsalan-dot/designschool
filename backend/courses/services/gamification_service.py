...
def get_gamification_summary(user):
    ...
    recent_transactions = XPTransaction.objects.filter(user=user).select_related('course', 'lesson')[:8]
    skill_rows = UserSkillProgress.objects.filter(user=user).select_related('skill').order_by('-xp')[:6]
