...
class PublicCourseApiTests(APITestCase):
    ...
    def test_public_course_detail_returns_real_module_and_technology_data(self):
        ...
        category = CourseTechnologyCategory.objects.create(
            course=course, name="Frontend", sort_order=0
        )
    ...
    def test_mark_lesson_completed_awards_xp_and_updates_course_progress(self):
        ...
        self.assertTrue(
            LessonProgress.objects.filter(user=student, lesson=lesson, status="completed").exists()
        )
        self.assertTrue(
            CourseProgress.objects.filter(user=student, course=course, xp_earned=30).exists()
        )
        self.assertTrue(
    ...
    def test_learning_path_api_starts_and_tracks_course_completion(self):
        ...
        self.assertTrue(
            UserLearningPathProgress.objects.filter(
                user=student, learning_path=learning_path
            ).exists()
        )
    ...
    def test_quiz_attempt_api_grades_answers_and_awards_xp_once(self):
        ...
        quiz = Quiz.objects.create(
            lesson=lesson, title="Hierarchy Check", passing_score=70, xp_reward=25
        )
        question = Question.objects.create(quiz=quiz, prompt="What creates stronger hierarchy?")
        correct_option = Option.objects.create(
            question=question, text="Clear contrast", is_correct=True
        )
        ...
        self.assertTrue(
            QuizAttempt.objects.filter(user=student, quiz=quiz, passed=True, score=100).exists()
        )
        self.assertTrue(
            LessonProgress.objects.filter(user=student, lesson=lesson, status="completed").exists()
        )
        ...
        self.assertEqual(
            XPTransaction.objects.filter(user=student, lesson=lesson, source="quiz_passed").count(),
            1,
        )
    ...
    def test_quiz_attempt_api_records_failed_attempt_without_xp(self):
        ...
        wrong_option = Option.objects.create(
            question=question, text="Random spacing", is_correct=False
        )
        self.client.force_authenticate(student)
        ...
        self.assertFalse(
            XPTransaction.objects.filter(user=student, lesson=lesson, source="quiz_passed").exists()
        )
    ...
    def test_public_course_detail_includes_platform_mentors_with_associated_mentor(self):
        ...
        associated_mentor = next(
            (mentor for mentor in payload["platformMentors"] if mentor["associated"]), None
        )
