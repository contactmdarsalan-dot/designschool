...
class ClassRecording(models.Model):
    ...
    video_url = models.URLField(help_text="Paste YouTube unlisted/public, Vimeo, Drive, or external video link")
    video_provider = models.CharField(max_length=20, choices=VIDEO_PROVIDER_CHOICES, default='external')
    youtube_video_id = models.CharField(max_length=32, blank=True, help_text="Auto-filled for YouTube links, including unlisted videos")
    is_unlisted = models.BooleanField(default=True, help_text="Mark private catalog videos that should only be visible to enrolled students")
    duration_seconds = models.PositiveIntegerField(blank=True, null=True, help_text="Optional duration used by the custom player UI")
