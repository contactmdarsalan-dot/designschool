...
def student_assignments(request):
    ...
    verified_enrollments = Enrollment.objects.filter(email=request.user.email, status="verified")
...
def submit_assignment(request, assignment_id):
    ...
    enrollment = Enrollment.objects.filter(
        email=request.user.email, course=assignment.course, status="verified"
    ).first()
    if not enrollment:
