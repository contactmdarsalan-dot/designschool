...
from .api_serializers import PublicBlogDetailSerializer, PublicBlogListSerializer
...
class PublicBlogListView(APIView):
    ...
    def get(self, request):
        page = parse_positive_int(
            request.query_params.get("page", 1), default=1, upper_bound=100000
        )
        limit = parse_positive_int(
            request.query_params.get("limit", 12), default=12, upper_bound=50
        )
        query = request.query_params.get("q", "").strip()
        category = request.query_params.get("category", "").strip()

        queryset = BlogPost.objects.filter(is_published=True, status="published").select_related(
            "category"
        )
...
class PublicBlogDetailView(APIView):
    ...
    def get(self, request, slug):
        ...
        related_serializer = PublicBlogListSerializer(
            related_queryset, many=True, context={"request": request}
        )
        return Response(
            {"data": {"post": serializer.data, "related_posts": related_serializer.data}}
        )
